...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Check for ZainCash payment query params
    ...
    if (paymentStatus && orderId) {
      if (paymentStatus === 'success') {
        showToast(lang === 'ar' ? `تم دفع طلبك ${orderId} بنجاح عبر ZainCash!` : `Payment successful for order ${orderId} via ZainCash!`, 'success');
        addLog(`ZainCash checkout successful for order ${orderId}`, 'success');
        setActiveTab('profile'); // Go to User Profile to see orders
      } else {
        const isInsufficient = reason && (reason.toLowerCase().includes('insufficient') || reason.includes('رصيد'));
        let failureTextAr = `فشلت عملية الدفع للطلب ${orderId} أو تم إلغاؤها.`;
        let failureTextEn = `Payment failed or cancelled for order ${orderId}.`;

        if (isInsufficient) {
          failureTextAr = `عفواً، فشلت عملية الدفع للطلب ${orderId}: الرصيد في محفظة زين كاش غير كافٍ.`;
          failureTextEn = `Payment failed for order ${orderId}: Insufficient balance in ZainCash wallet.`;
        } else if (reason) {
          failureTextAr = `فشلت عملية الدفع للطلب ${orderId}: ${reason}`;
          failureTextEn = `Payment failed for order ${orderId}: ${reason}`;
        }

        showToast(failureTextAr, 'info');
        addLog(`ZainCash checkout failed for order ${orderId}. Reason: ${reason || 'Insufficient funds or cancelled'}`, 'warning');
        setActiveTab('cart'); // Keep in cart
      }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500/30 selection:text-white overflow-x-hidden">
      ...
      {/* Clients & Achievements Stats Section */}
