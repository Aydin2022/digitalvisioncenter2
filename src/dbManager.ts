...
export const db = {
  ...
  resetPasswordWithEmail: (email: string, newPassword: string): { success: boolean; error?: string } => {
    const users = db.getUsers();
    ...
    return { success: false, error: 'البريد الإلكتروني المدخل غير مسجل في النظام' };
  }
};
